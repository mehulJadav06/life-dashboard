  ...
  return (
    <header className="fixed top-0 right-0 left-[240px] h-16 z-30 flex items-center justify-between px-6
      ...
      <div className="flex items-center gap-2">
        ...
        {/* Sync status */}
        {syncing ? (
          <div className="flex items-center gap-1.5 text-brand-400 text-xs px-2">
            <RefreshCw className="w-3.5 h-3.5 animate-spin" />
            <span className="hidden sm:inline">Syncing…</span>
          </div>
        ) : error ? (
          <div className="flex items-center gap-1.5 text-red-400 text-xs px-2" title={error}>
            <WifiOff className="w-3.5 h-3.5" />
            <span className="hidden sm:inline">Sync error</span>
          </div>
        ) : (
          <div className="flex items-center gap-1.5 text-emerald-400 text-xs px-2">
        ...
        {user && (
          <button
            onClick={() => signOut()}
            className="btn-ghost p-2 rounded-lg text-gray-500 hover:text-red-400"
            title="Sign out"
            id="topbar-signout"
          >
